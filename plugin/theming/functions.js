...
module.exports = {
    ...
    injectThemes: function (addBase, config, themes, colorFunction) {
      ...
      // add custom themes
      if (Array.isArray(config("daisytheming.themes"))) {
        config("daisytheming.themes").forEach((item) => {
      ...
      let themeOrder = []
      if (Array.isArray(config("daisytheming.themes"))) {
        config("daisytheming.themes").forEach((theme) => {
      ...
      } else if (config("daisytheming.themes") === true) {
      ...
      themeOrder.forEach((themeName, index) => {
        if (index === 0) {
          ...
          // auto dark
          if (config("daisytheming.darkTheme")) {
            if (
              themeOrder[0] !== config("daisytheming.darkTheme") &&
              themeOrder.includes(config("daisytheming.darkTheme"))
            ) {
              themesToInject["@media (prefers-color-scheme: dark)"] = {
                [":root"]: includedThemesObj[`[data-theme=${config("daisytheming.darkTheme")}]`],
              }
            }
          } else if (config("daisytheming.darkTheme") === false) {
